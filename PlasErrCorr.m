function [hc,H,Ac]=PlasErrCorr(ARAdir,Disp,Load,Er,LoadAmp)
    arguments
        ARAdir char %directory with area function
        Disp cell %displacement
        Load cell %load
        Er double %reduced modulus
        LoadAmp cell %load amplitude
    end
    ...
    for FileNumber=1:nfiles
        for Point=1:length(Load{FileNumber})
            ...
            hc{FileNumber}(Point)=Disp{FileNumber}(Point)-10;
            ...
            H{FileNumber}(Point)=((Load{FileNumber}(Point)+LoadAmp{FileNumber}(Point))*1e-6)...
                /(Ac{FileNumber}(Point)*1e-9);
